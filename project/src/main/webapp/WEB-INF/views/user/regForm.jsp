    	...
    	$(document).ready(function(){
    		...
    		//EMAIL CHECK BUTTON ON CLICK
 			...
 			$("#response-email-check-number-btn").on("click", function(){	//EMAIL CHECK TRUE&FALSE INPUT BOX 
 				...
 				if($("#emailCheckInput").val() == emailCheckCode) {
 					...
 					$("#userEmail").attr("readonly", true);
 					$("#emailCheckInput").attr("readonly",true);
 			...
 			$("#userPostcode").blur(function() {
    			if($.trim($("#userPostcode").val()).length <= 0) {
    				//alert("우편번호를 입력해주세요.");
    				$(".userPostcode_check").text("우편번호를 입력해주세요");
    				//$("#userPostcode").focus();
    				return;
    			}else{
    				$(".userPostcode_check").text("");
    				return;
    			}
 			});
 			...
 			$("#userAddress").blur(function() {
 				if($.trim($("#userAddress").val()).length <= 0) {
    				//alert("상세주소를 입력해주세요.");
    				$(".userAddress_check").text("기본주소를 입력해주세요");
    				//$("#userDetailAddress").focus();
    				return;
    			}else{
    				$(".userAddress_check").text("");
    				return;
    			}
 			})
 			*/
    		...
    		$("#btnUserReg").on("click", function(){
    			...
    			if($("#userPwd1").val() != $("#userPwd2").val()) {
    				//alert("비밀번호 확인이 일치하지않습니다.");
    				$(".userPwd2_check").text("비밀번호 확인이 일치하지않습니다");
    				$("#userPwd2").val("");
    				$("#userPwd2").focus();
    				return;
    			}
    			...
    			if(!fn_validateEmail($("#userEmail").val())) {
    				//alert("올바른 이메일을 입력해주세요.");
    				$(".userEmail_check").text("올바른 이메일을 입력해주세요");
    				$("#userEmail").focus();
    				return;
    			}
    			...
    			if($.trim($("#userPostcode").val()).length <= 0) {
    				//alert("우편번호를 입력해주세요.");
    				$(".userPhoneNo_check").text("우편번호를 입력해주세요");
    				$("#userPostcode").focus();
    				return;
    			}
    			...
    			if($.trim($("#userDetailAddress").val()).length <= 0) {
    				//alert("상세주소를 입력해주세요.");
    				$(".userPhoneNo_check").text("상세주소를 입력해주세요");
    				$("#userDetailAddress").focus();
    				return;
    			}
    			*/
...
<%@include file="/WEB-INF/views/include/headerNavigation.jsp" %>
 ...
 <!-- @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@도로명주소 입력창@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ -->		
				...
				<!--<div class="userPostcode_check check_text"></div>-->
	            <div class="input_div">
	                <!-- 기본주소 -->
					<input type="text" id="userAddress" class="required" placeholder="주소"><br>
	            </div>
	            <!--<div class="userAddress_check check_text"></div> -->
	            <div class="input_div">
	                <!-- 상세주소 -->
